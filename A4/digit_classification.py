...
def backpropagation(x, t, W, B):
    ...
    # TODO: YOUR CODE STARTS HERE
#    print([np.zeros(b.shape) for b in B])
    delta = [np.zeros(b.shape) for b in B]
    delta[-1] = np.dot(derivative_activation(z_s[-1]),(t-a_s[-1]))
    d_b[-1] = delta[-1] * -1
    
    for i in range(len(B[0])):
        #delta i <- g'(ini) sum(w*deltaj)
        delta[0][i] = np.dot(derivative_activation(z_s[1][i]), np.sum(W[-1][0][i] * delta[-1]))
        d_b[0][i] = delta[0][i] * -1
        d_w[-1][0][i] = np.dot(delta[-1],a_s[-2][i])
    
    for i in range(len(B[0])):
        #wij <- wij + ai*deltaj
        d_w[0][i] = d_w[0][i] + np.dot(delta[0][i],np.transpose(a_s[0]))
...
#    print (a_s[2].shape)
    ...
    ## set print sample values to false
